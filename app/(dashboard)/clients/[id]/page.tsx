...
export default function ClientProfilePage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalLoans = [
      ...new Set(client.instalments.map((i: any) => i.loanId)),
    ].length;
    const activeLoans = [
      ...new Set(
        client.instalments
          .filter(
            (i: any) =>
              i.loan.status === "APPROVED" || i.loan.status === "ACTIVE",
          )
          .map((i: any) => i.loanId),
      ),
    ].length;

    const totalPaid = client.instalments.reduce(
      (acc: number, inst: any) => acc + Number(inst.paidAmount),
      0,
    );
    const totalDue = client.instalments.reduce(
  ...
  if (!client)
    return (
      <div className="p-10 text-center font-medium">Client not found.</div>
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-6">
          <Button
            ...
            size="icon"
            onClick={() => router.back()}
            className="rounded-full hover:bg-primary/10 transition-colors h-12 w-12 border shrink-0"
          >
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground font-semibold flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-primary" /> Member ID:{" "}
                {client.clientNo} <span className="opacity-40">|</span>{" "}
                <CreditCard className="w-4 h-4" /> NIC: {client.nic}
              </p>
            </div>
          </div>
        </div>
        ...
        <Card className="border-none shadow-xl bg-card/60 backdrop-blur-md overflow-hidden relative group">
          ...
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-black text-slate-800">
              {stats?.totalLoans} Applications
            </p>
            <p className="text-[10px] text-primary font-bold mt-2">
              {stats?.activeLoans} Currently Active
            </p>
          </CardContent>
        </Card>
        <Card className="border-none shadow-xl bg-card/60 backdrop-blur-md overflow-hidden relative group">
          ...
          <Card className="border-none shadow-xl bg-card/60 backdrop-blur-md">
            ...
            <CardContent className="p-4 space-y-3">
              {client.groupMembers?.map((gm: any) => (
                <div
                  key={gm.id}
                  className="p-4 rounded-xl border bg-white/50 hover:bg-primary/5 hover:border-primary/20 transition-all cursor-pointer group"
                  onClick={() => router.push(`/groups/${gm.groupId}`)}
                >
              ...
              {client.groupMembers?.length === 0 && (
                <p className="text-center py-4 text-xs font-bold text-muted-foreground italic">
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      ...
                      {Object.values(
                        client.instalments.reduce((acc: any, inst: any) => {
                          if (!acc[inst.loanId]) {
                            acc[inst.loanId] = {
                              id: inst.loan.id,
                        ...
                        >
                          ...
                          <TableCell className="text-right font-black text-primary">
                            Rs. {Number(loan.lent).toLocaleString()}
                          </TableCell>
                      ...
                      {client.instalments.length === 0 && (
                        <TableRow>
                          <TableCell
                            colSpan={5}
                            className="h-32 text-center text-muted-foreground italic font-bold"
                          >
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {client.documents?.map((doc: any) => (
                  <Card
                    key={doc.id}
                    className="border shadow-sm hover:shadow-md transition-shadow bg-white/50 overflow-hidden group"
                  >
                ...
                {client.documents?.length === 0 && (
                  <div className="col-span-full py-20 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center opacity-40">
                    ...
                    <p className="text-sm font-black uppercase tracking-widest text-center">
                      No documents found <br /> for this client
                    </p>
                  </div>
                )}
              </div>
